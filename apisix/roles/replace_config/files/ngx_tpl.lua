...
http {
    ...
    {% else %}
     ...
     '"http_uuid": "$http_uuid", '
     '"http_x_forwarded_uuid": "$upstream_http_uuid", '
     '"http_timestamp": "$http_timestamp", '
     '"http_x_forwarded_timestamp": "$upstream_http_timestamp", '
     '"proxy_host": "$proxy_host" } }';
    uninitialized_variable_warn off;
    access_log {* http.access_log *} json ;
    ...
    server {
        ...
        include /usr/local/openresty/nginx/conf/lua/*.conf;
        set $upstream_host "";
        set $request_merchant "";
        set $header_param "";
